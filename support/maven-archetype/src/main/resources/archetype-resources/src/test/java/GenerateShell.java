...
import org.springside.modules.mybatis.generator.CustomerMyBatisGenerator;
...
public class GenerateShell {
    ...
    public static void main(String[] args) {
        ...
        String path = GenerateShell.class.getClassLoader().getResource("generatorXML").getFile();
        ...
        for (File configFile : dir.listFiles()) {
            ...
            try {
                ...
                for (Context context : config.getContexts()) {
                    JDBCConnectionConfiguration jdbcContext = context.getJdbcConnectionConfiguration();
                    jdbcContext.setConnectionURL(jdbcContext.getConnectionURL());
                    jdbcContext.setUserId(jdbcContext.getUserId());
                    jdbcContext.setPassword(jdbcContext.getPassword());
                    jdbcContext.setDriverClass(jdbcContext.getDriverClass());
                }
                DefaultShellCallback shellCallback = new DefaultShellCallback(Boolean.TRUE);

                CustomerMyBatisGenerator myBatisGenerator = new CustomerMyBatisGenerator(config, shellCallback, warnings);
